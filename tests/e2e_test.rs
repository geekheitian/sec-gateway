...
fn test_e2e_pii_masking_flow_chinese_id() {
    ...
    for (idx, pii) in detections.iter().enumerate() {
        ...
        assert!(!masked_body.contains(&pii.value), "Original PII should NOT be in masked body");
        assert_ne!(token, "110101199001011234", "Encrypted ID should differ from original");
...
fn test_e2e_pii_masking_flow_api_key() {
    ...
    for pii in &detections {
        ...
        assert!(!masked_body.contains("sk-proj-AbCdEf1234567890XyZ"), 
            "Original API key should NOT be in masked body");
    }
    
    assert!(masked_body.contains("[HASH:"), "Should contain hash placeholder");
...
fn test_e2e_multiple_pii_types_single_request() {
    ...
    all_matches.extend(detect_chinese_id(body).into_iter()
    ...
    assert!(!masked_body.contains("110101199001011234"), "Original ID should NOT be in masked body");
    assert!(!masked_body.contains("13812345678"), "Original phone should NOT be in masked body");
    assert!(!masked_body.contains("test@example.com"), "Original email should NOT be in masked body");
    assert!(!masked_body.contains("sk-proj-Test1234567890abcdef"), "Original API key should NOT be in masked body");
...
fn test_e2e_session_isolation() {
    ...
    vault.store(session1, "token1".to_string(), "value1".to_string()).unwrap();
    vault.store(session2, "token1".to_string(), "value2".to_string()).unwrap();
    
    assert_eq!(vault.retrieve(session1, "token1").unwrap(), Some("value1".to_string()));
    assert_eq!(vault.retrieve(session2, "token1").unwrap(), Some("value2".to_string()));
...
fn test_e2e_zero_data_leakage() {
    ...
    for pii_value in original_pii_values {
        ...
        if !all_matches.is_empty() {
            ...
            assert!(!masked_body.contains(pii_value), 
                "PII '{}' should NOT appear in masked body", pii_value);
        }
    }
}
...
fn test_e2e_vault_bulk_operations() {
    ...
    for i in 0..100 {
        ...
        vault.store(session_id, token.clone(), value.clone()).unwrap();
    }
    ...
    for i in 0..100 {
        ...
        assert_eq!(vault.retrieve(session_id, &token).unwrap(), Some(expected_value));
    }
}
