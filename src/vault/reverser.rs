...
impl Reverser {
    ...
    pub fn restore_response(&self, session_id: &str, body: &str) -> Result<String, String> {
        ...
        for pattern in patterns {
            ...
            let matches: Vec<_> = re.find_iter(&result).map(|m| (m.start(), m.end(), m.as_str().to_string())).collect();
    ...
    pub fn restore_tokens(&self, session_id: &str, tokens: &HashSet<String>) -> Result<HashSet<String>, String> {
...
mod tests {
    ...
    fn test_restore_response_basic() {
        let vault = PrivacyVault::new();
        vault.store("session1", "165455343746803619".to_string(), "110101199001011234".to_string()).unwrap();
    ...
    fn test_restore_response_email() {
        let vault = PrivacyVault::new();
        vault.store("session1", "[REDACTED_EMAIL_001]".to_string(), "test@example.com".to_string()).unwrap();
    ...
    fn test_restore_tokens_bulk() {
        let vault = PrivacyVault::new();
        vault.store("session1", "165455343746803619".to_string(), "value1".to_string()).unwrap();
        vault.store("session1", "[REDACTED_EMAIL_001]".to_string(), "value2".to_string()).unwrap();
        vault.store("session1", "[HASH:abcdef1234567890]".to_string(), "value3".to_string()).unwrap();
