...
mod tests {
    ...
    fn test_fpe_preserves_format() {
        ...
        let encrypted_str: String = cipher_vec.iter().map(|&d| ((d as u8) + b'0') as char).collect();
        println!("Original: {}", id);
    ...
    fn test_fpe_different_inputs() {
        ...
        let plaintext1: Vec<u16> = id1.chars().map(|c| c.to_digit(10).unwrap() as u16).collect();
        let plaintext2: Vec<u16> = id2.chars().map(|c| c.to_digit(10).unwrap() as u16).collect();
        
        let ciphertext1 = ff1.encrypt(tweak, &FlexibleNumeralString::from(plaintext1)).unwrap();
        let ciphertext2 = ff1.encrypt(tweak, &FlexibleNumeralString::from(plaintext2)).unwrap();
