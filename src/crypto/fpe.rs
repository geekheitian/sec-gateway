...
impl FPECipher {
    ...
    pub fn encrypt(&self, plaintext: &str) -> Result<String, String> {
        let digits: Vec<u16> = plaintext.chars()
        ...
        let ciphertext = self.ff1.encrypt(b"", &plaintext_numeral)
            .map_err(|e| format!("Encryption failed: {:?}", e))?;

        let cipher_vec: Vec<u16> = ciphertext.into();
        Ok(cipher_vec.iter().map(|&d| char::from_digit(d as u32, self.radix).unwrap()).collect())
    }

    pub fn decrypt(&self, ciphertext: &str) -> Result<String, String> {
        let digits: Vec<u16> = ciphertext.chars()
            .map(|c| c.to_digit(self.radix).ok_or("Invalid digit"))
            .collect::<Result<Vec<_>, _>>()
        ...
        let plaintext = self.ff1.decrypt(b"", &ciphertext_numeral)
            .map_err(|e| format!("Decryption failed: {:?}", e))?;

        let plain_vec: Vec<u16> = plaintext.into();
        Ok(plain_vec.iter().map(|&d| char::from_digit(d as u32, self.radix).unwrap()).collect())
    }
}
...
mod tests {
    ...
    fn test_fpe_deterministic() {
        ...
        let encrypted1 = cipher.encrypt(input).unwrap();
        let encrypted2 = cipher.encrypt(input).unwrap();
    ...
    fn test_fpe_different_keys_different_output() {
        ...
        let input = "1234567890";
        let encrypted1 = cipher1.encrypt(input).unwrap();
        let encrypted2 = cipher2.encrypt(input).unwrap();
    ...
    fn test_fpe_invalid_input() {
        ...
        let invalid = "abc123";
        let result = cipher.encrypt(invalid);
    ...
    fn test_fpe_empty_string() {
        ...
        let empty = "";
        let result = cipher.encrypt(empty);
    ...
    fn test_fpe_long_input() {
        ...
        let encrypted = cipher.encrypt(&long_input).unwrap();
        let decrypted = cipher.decrypt(&encrypted).unwrap();
